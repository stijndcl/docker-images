...
if 'code' in form:
  ...
  code = open('testcode/testcode.py').read()

#overwrite input()
if 'input' in form:
  options.raw_input_lst_json = form['input'].value
...
if options.usercode:
  ...
  if options.js_varname:
    ...
    print(pg_logger.exec_script_str_local(code, options.raw_input_lst_json, options.cumulative, options.heapPrimitives, js_var_finalizer))
  else:
    print(pg_logger.exec_script_str_local(code, options.raw_input_lst_json, options.cumulative, options.heapPrimitives, json_finalizer))
